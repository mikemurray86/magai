...
cargo test               # run tests (no test suite exists yet — add #[cfg(test)] modules as needed)
cargo test <name>        # run a single test by name/substring
cargo clippy             # lint
cargo fmt                # format
```

There is no test suite, README, lint config, or CI in the repo currently —
don't assume conventions beyond what's in the source.
...
- **`ai::run_agent`** (src/ai.rs) — owns the LLM agent, conversation history,
  tool server, and hook runner. Receives `AgentCommand`s (user message, model
  switch, approve/deny tool call, clear, undo, …) and emits `AiEvent`s
  (token, tool-call start/result, errors, …).
- **`ui::App`** (src/ui.rs) — the ratatui event loop (`App::run`). Owns all
  rendering and input handling, sends `AgentCommand`s, and drains `AiEvent`s
  each frame via `poll_ai_events`.
...
### Provider abstraction (`ai.rs`)

Each LLM provider (Ollama, OpenAI, Anthropic, Groq; Gemini is stubbed/unsupported)
is wrapped into a type-erased `DynAgent` (a boxed streaming-chat closure) so the
rest of the code doesn't care which `rig` client backs the current model.
`resolve_agent` picks a provider based on the config's named models, falling
back to treating the alias as a raw Ollama model name. Streaming responses are
normalized into `OurItem` (`map_item`) and then driven by `drive_stream`, which
`tokio::select!`s between the model stream and incoming `AgentCommand`s (so
cancel/approve/deny can interrupt an in-flight turn).
...
registered in `build_tool_server` (ai.rs) with an explicit danger flag.
...
permission mode, context window, MCP servers, hooks). Missing/unparsable config
silently falls back to `Config::default()`.

### UI rendering (`ui.rs`)

`App::draw` lays out title/separator/history/input regions each frame and
renders chat history as a flat `Vec<ChatMessage>` converted to `Line`s by
`build_lines`. Two wrapping paths exist and must both stay overflow-safe:
plain text (`word_wrap`, used for user/system/tool messages, measured in UTF-8
byte length — a safe upper bound on terminal display width) and markdown
(`markdown_to_static_lines` → `wrap_styled_line`, used for assistant messages,
which preserves per-character styles across wrap points and hard-breaks tokens
wider than the available width). `view_width`/`view_height` are recomputed from
the actual `history_area` each frame and drive both wrapping and scroll-offset
math — don't hardcode terminal dimensions.
